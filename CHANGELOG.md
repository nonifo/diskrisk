...
### Diskrisk — risk engine
- `risk_engine.py`: **classic** (code default) / **hybrid** / **stats** via `DISKRISK_RISK_ENGINE`
- Hybrid policy: **media** / **interface** / **advisory** kinds; MultiZone advisory ≠ FAIL
- UDMA_CRC / timeouts: GROWING → warn; no increase ≥ N days → historical
  (`DISKRISK_INTERFACE_HISTORICAL_DAYS`, default 14)
- GrownDefect / Realloc: GROWING → critical; stable ≥ N days → historical
  (`DISKRISK_MEDIA_SCAR_HISTORICAL_DAYS`, default 30 in example config).
  Pending / OfflineUnc never demoted
- `/json` includes `engine`, `policy`, and `classic_findings` for A/B when not classic
- UI: kind chips, legend, scar progress badge (`stable 16d/14d`)
- Docs: SMART-risk-manual §8b (TR-54 / DevStat / smartmontools decoder)

### History
- Retain `SMART_RISK_HISTORY_DAYS` (default **365**), max `SMART_RISK_HISTORY_PER_DAY` (default **2**)
- `first_seen` / `first_value` kept when old samples age out

### smart_collect.py
- Optional `smartctl -x -j` overlays for engine=`stats` (Device Statistics / self-test / error log)
